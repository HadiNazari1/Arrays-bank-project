...
2. Find Sarah's dog and log to the console whether it's eating too much or too little. HINT: Some dogs have multiple users, so you first need
to find Sarah in the owners array, and so this one is a bit tricky (on purpose) 🤓
3. Create an array containing all owners of dogs who eat too much (ownersTooMuch) and an array with all owners of dogs 
who eat too little (ownersTooLittle).
...
*/
  ...
  const dogSarah = dogs.find(dog=> dog.owners.inculdes('Sarah'));
  const dogSarah=dogs.find(dog=> dog.owners.includes('Sarah'));
console.log(`Sarah's dog eats too ${dogSarah.curFood>dogSarah.recfood ? 'much':'little'}`)
  console.log(`Sara dog's eats too ${dogSarah.curfood> dogSarah.recfood ? 'much' :'little'}`)

  const eatTolittle= dogs.filter(dog=> dog.curFood<dog.recfood).flatMap(dog=> dog.owners)

  const groupedByOwner= Object.groupBy(dogs, dog=> 

  const dogsSorted= dogs.toSorted((a,b) => a.recfood-b.recfood)
console.log(doggsSored)
  */
/*
const dogs = [
  { weight: 22, curFood: 250, owners: ['Alice', 'Bob'] },
...
const breeds = [
  ...
  const dogBothActivities = breeds.find(breed=> breed.activities.includes('running')&& breed.activities.includes('fetch')).breed;
    ...
    const allActivities = breeds.flatMap(breed => breed.activities);
    console.log(allActivities);
cosnt allActivities=breed.flatMap(breed=> breed.activities);
    console.log(allActivities);
    const uniqueActivities = new Set (allActivities);
